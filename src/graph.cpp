...
Graph::AntimagicResult Graph::is_antimagic(int increment) {
    ...
    if (get_isolated().size() > 1)
        return AntimagicResult{false, true, false, 0};
    ...
    while (gen.next()) {
        ...
        if (antimagic)
            return AntimagicResult{true, false, iteration <= RANDOM_PERMUTATIONS_COUNT, iteration};
        iteration++;
    }
    // return false if antimagic phi was never found
    return AntimagicResult{false, false, false, iteration};
}
